...
export function MatchWizard({
  ...
  const categories =
    data?.facets?.categories?.map((item) => item.name).filter(Boolean) || [];
  ...
  useEffect(() => {
    ...
    const nextStep = edit
      ? 0
      : Math.min(findFirstIncompleteMatchStep(answers), questions.length - 1);
          ...
          {processing ? (
            ...
            <AnimatePresence mode="wait">
              ...
              >
                ...
                <p>{q.copy}</p>
                <div
                  className={`quiz-options ${q.options.length <= 3 ? "quiz-options-grid-1" : q.options.length > 4 ? "quiz-options-grid-3" : ""}`}
                >
                  {q.options.map((option) => (
                    ...
                    >
                      ...
                      <span className="option-check">
                        {answers[q.id] === option.value && <Check size={15} />}
                      </span>
                    </button>
                  ))}
                </div>
                <button
                  className="btn"
                  disabled={!hydrated || !answers[q.id]}
                  onClick={next}
                >
