...
import { ActionForm, UploadForm, type UploadedFileInfo } from "./action-form";
import {
  AvailabilityPanel,
  PackagesPanel,
  record,
...
export function TrainerOnboarding() {
  ...
  const identity =
    credentials.find((item) => str(item, "type") === "IDENTITY") ||
    (str(trainer, "cnicUploadId")
      ? {
          uploadId: str(trainer, "cnicUploadId"),
          verificationStatus:
            str(trainer, "identityVerificationStatus") || "PENDING",
        }
      : null);
  const certificates = credentials.filter(
    (item) => str(item, "type") === "CERTIFICATION",
  );
  ...
  if (loading && !data)
    return (
      <div className="container section">
        <div className="panel">Loading trainer onboarding…</div>
      </div>
    );
  if (error && !data)
    return (
      <div className="container section">
        <div className="panel">
          <h1>We could not load onboarding.</h1>
          <p>{error}</p>
    ...
    return (
      <div className="container section narrow-page">
        <section className="panel onboarding-complete">
          ...
          <p>
            Your public trainer profile is active. You can keep your profile,
            services and availability up to date from the dashboard.
          </p>
          <button className="btn" onClick={() => router.push("/trainer")}>
    ...
    return (
      <div className="container section narrow-page">
        <section className="panel onboarding-complete">
          ...
          <p>
            We’ll keep updates in your in-app notifications. Your profile
            remains private until identity, certification and the application
            are approved.
          </p>
          <button
            className="btn outline"
            onClick={() => router.push("/trainer/application")}
          >
  ...
  const hasCnic = Boolean(
    identityUpload || identity || str(trainer, "cnicUploadId"),
  );
  const identityMissing = [!hasCnic && "CNIC document upload"].filter(
    Boolean,
  ) as string[];
  ...
  const checklist = [
    ...
    },
    {
      ...
      missing: packages.some((item) => item.active !== false)
        ? []
  ...
  return (
    <div className="container section onboarding-shell">
      <header className="page-heading onboarding-heading">
        ...
        <p>
          Complete each section, save your progress, then submit once for admin
          review. You can edit normal profile details later from your dashboard.
        </p>
      ...
      <div
        className="onboarding-progress desktop-onboarding-progress"
        aria-label="Trainer onboarding steps"
      >
      ...
      <div
        className="mobile-onboarding-header"
      ...
      >
        <div className="mobile-step-info">
          ...
          <select
            aria-label="Jump to application section"
            value={step}
      ...
      {step === 0 && (
        <section className="panel onboarding-card">
          ...
          <p>
            This is what customers use to understand who you coach and how you
            can help. Your photo and profile details are saved separately, and
          ...
          <div className="onboarding-upload onboarding-upload-first">
            ...
            <p className="muted">
              Upload a clear professional headshot. Once saved, it appears here
              whenever you return.
            </p>
            <UploadForm
              purpose="PUBLIC"
            ...
            fields={[
              {
                ...
                value: str(trainer, "displayName") || str(account, "name"),
                required: true,
              },
              {
                name: "headline",
                ...
                required: true,
                hint: "Example: Online strength & body transformation coach",
              },
              {
                name: "biography",
                label: "About you",
                ...
                required: true,
                hint: "Minimum 100 characters before final submission.",
              },
              {
              ...
              },
              {
                ...
                type: "select",
                options: categories,
                value: str(trainer, "category") || categories[0],
                required: true,
              },
              {
                name: "specialties",
          ...
          <div className="onboarding-next">
            ...
            <button
              className="btn"
              onClick={() => setStep(1)}
              disabled={profileMissing.length > 0}
            >
          ...
          <div className="cnic-upload-zone">
            <div className="cnic-upload-header">
              ...
              <small className="muted">
                Supported formats: JPG, PNG, WebP, PDF (Up to 4 MB)
              </small>
            </div>
            ...
            {hasCnic ? (
              <div className="cnic-success-badge" role="status">
                ...
                <div>
                  ...
                  <p className="muted">
                    {identityUpload?.name ||
                      "Your identity document is stored securely for verification."}
                  </p>
                  {(identityUpload?.url ||
                    str(identity || {}, "uploadId") ||
                    str(trainer, "cnicUploadId")) && (
                    <a
                      ...
                      href={
                        identityUpload?.url ||
                        `/api/media/${str(identity || {}, "uploadId") || str(trainer, "cnicUploadId")}`
                      }
                      target="_blank"
                      rel="noreferrer"
                    >
      ...
      {step === 2 && (
        <section className="panel onboarding-card">
          ...
          <p>
            Add at least one professional qualification. Each certificate is
            reviewed independently by an admin.
          </p>
          ...
          <UploadForm
            purpose="PRIVATE"
            pendingUpload={certificateUpload}
            onUploaded={setCertificateUpload}
          />
          <ActionForm
            ...
            fields={[
              ...
              },
              {
                ...
                label: "Credential / licence number",
              },
            ...
            transform={(value) => ({
              ...value,
              type: "CERTIFICATION",
              uploadId: certificateUpload?.id,
              credentialNumber: value.credentialNumber || undefined,
          ...
          {!certificateUpload && (
            <p className="onboarding-guidance">
              Upload the certificate file before adding a new certification.
            </p>
          )}
        </section>
      )}
      ...
      {step === 3 && (
        <div className="onboarding-card-stack">
          <div className="panel">
            ...
            <p>
              Define your session duration, single session price, and packages.
              Prices are entered in PKR and existing bookings keep their
          ...
          <div className="onboarding-next">
            <button
              className="btn"
              onClick={() => setStep(4)}
              disabled={!packages.some((item) => item.active !== false)}
            >
      ...
      {step === 4 && (
        <div className="onboarding-card-stack">
          <div className="panel">
            ...
            <p>
              Set your primary operating timezone and the weekly times you can
              deliver online sessions. Customers automatically see booking slots
              converted to their local time.
            </p>

            <div
              className="timezone-setting-box"
            ...
            >
              <label className="field">
                  ...
                  onChange={async (e) => {
                    ...
                    try {
                      await api("trainer/profile", {
                        displayName:
                          str(trainer, "displayName") || str(account, "name"),
                        ...
                        trainingGoals: [
                          str(trainer, "category") || categories[0],
                        ],
                        timezone: newTz,
                      });
                      await reload();
                ...
                <small className="muted">
                  Your weekly availability hours below will be interpreted using
                  this timezone.
                </small>
              </label>
              {tzError && (
          ...
          <AvailabilityPanel
            data={{ ...data, timezone: str(trainer, "timezone"), rules }}
            reload={reload}
          />
          <div className="onboarding-next">
            <button
      ...
      {step === 5 && (
        <section className="panel onboarding-card review-submit-card">
          ...
          <h2>Review & submit</h2>
          <p>
            Submitting locks the application for review. You will receive status
            updates in your Spotter notifications.
          </p>
          <p className="onboarding-summary">
            <strong>
              {completedCount} of {checklist.length} sections complete.
            </strong>{" "}
            Each incomplete row tells you exactly what remains.
          </p>
            ...
            {checklist.map((item) => (
              <div key={item.label} className={item.done ? "done" : "missing"}>
                ...
                <div>
                  ...
                  {!item.done && (
                    <small>Still needed: {item.missing.join(", ")}.</small>
                  )}
                </div>
                {!item.done && (
                  <button
          ...
          {!ready && (
            <p className="onboarding-guidance">
              Finish the highlighted sections above before submitting. Your
              completed sections and uploaded files are already saved.
            </p>
          )}
        </section>
      )}
